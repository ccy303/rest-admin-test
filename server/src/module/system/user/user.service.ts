...
import { CacheServiceRep as CacheService } from "src/module/monitor/_cache/cache.service";
...
export class UserService {
    constructor(
        ...
        private readonly cacheService: CacheService // private readonly deptService: DeptService
    ...
    async login(user: LoginDto, clientInfo: ClientInfoDto) {
        const enable = await this.sysConfigService.findOneByConfigKey("sys.account.captchaEnabled");

        const captchaEnabled: boolean = enable.data.configValue === "true";

        if (captchaEnabled) {
            const code = await this.cacheService.get(CacheEnum.CAPTCHA_CODE_KEY + user.uuid);
            if (!code) return ResultData.fail(500, `验证码已过期`);
            if (Number(code) !== Number(user.code)) return ResultData.fail(500, `验证码错误`);
        }
        const data = await this.userRepo.findOne({ where: { userName: user.username }, select: ["userId", "password"] });
        ...
        await this.userRepo.update({ userId: data.userId }, { loginDate: loginDate });

        const uuid = GenerateUUID();
        const { expire, refreshExpire } = this.ConfigService.get("jwt");
        const token = {
            expire,
            token: this.generateToken({ uuid: uuid, userId: userData.userId }, false),
            refreshExpire,
            refreshToken: this.generateToken({ uuid: uuid, userId: userData.userId }, true)
        };
        ...
        this.cacheService.set(`${CacheEnum.LOGIN_TOKEN_KEY}${uuid}`, metaData, LOGIN_TOKEN_EXPIRESIN);
    ...
    async getUserinfo(userId: number): Promise<any> {
        // 联查部门详情
        const entity = this.userRepo.createQueryBuilder("user");
        entity.where({ userId: userId });
        entity.leftJoinAndMapOne("user.dept", SysDeptEntity, "dept", "dept.deptId = user.deptId");
        const data: any = await entity.getOne();

        const roleIds = await this.getRoleIds([userId]);
        const roles = await this.roleService.findRoles({
            where: { delFlag: DelFlagEnum.NORMAL, roleId: In(roleIds) }
        });
        const postIds = (await this.sysUserWithPostEntityRep.find({ where: { userId: userId }, select: ["postId"] })).map(
            item => item.postId
        );
        const posts = await this.sysPostEntityRep.find({
            where: { delFlag: DelFlagEnum.NORMAL, postId: In(postIds) }
        });
    ...
    async changeStatus(changeStatusDto: ChangeStatusDto) {
        ...
        const res = await this.userRepo.update({ userId: changeStatusDto.userId }, { status: changeStatusDto.status });
     ...
     * @return 令牌
     */
    generateToken(payload: { uuid: string; userId: number }, isRefresh = false, expiresIn?: number): string {
        const { expire, refreshExpire, secret } = this.ConfigService.get("jwt");
        const tokenInfo = { isRefresh, ...payload };
        return this.jwtService.sign(tokenInfo, {
            secret,
            expiresIn: expiresIn !== undefined ? expiresIn : isRefresh ? refreshExpire : expire
        });
    }

...
export class MainController {
    ...
    login(@Body() user: LoginDto, @Request() req) {
        ...
        const clientInfo = { userAgent: req.headers["user-agent"], ipaddr: req.ip, browser, os, loginLocation: "" };
    ...
    async captchaImage() {
        //是否开启验证码
        const enable = await this.configService.findOneByConfigKey("sys.account.captchaEnabled");
        const captchaEnabled: boolean = enable.data.configValue === "true";
        const data = { captchaEnabled, img: "", uuid: "" };
        try {
            if (captchaEnabled) {
